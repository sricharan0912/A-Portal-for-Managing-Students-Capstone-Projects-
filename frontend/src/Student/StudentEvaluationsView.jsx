 ...
 * Matches the design of ClientEvaluationsView with icons, days left, and filter tabs.
 */
export default function StudentEvaluationsView({ studentId }) {
  ...
  const filteredEvaluations = evaluations.filter((e) => {
    ...
    // Time-based filter
    if (filter === "upcoming") {
      if (evalDate < today || e.status === "completed" || e.status === "cancelled") return false;
    }
    if (filter === "past") {
      if (evalDate >= today && e.status !== "completed") return false;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Track your scheduled and completed project evaluations
          </p>
        </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-slate-300 pl-10 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        />
      </div>
      ...
      {filteredEvaluations.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-800 mb-2">
            {searchTerm ? "No Matching Evaluations" : `No ${filter === "all" ? "" : filter} Evaluations`}
          </h3>
          <p className="text-slate-500">
            {searchTerm 
              ? "Try adjusting your search terms" 
              : "Your project evaluations will appear here once they are scheduled."}
          </p>
        </div>
      ) : (
